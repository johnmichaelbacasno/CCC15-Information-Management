...
class Main:
    ...
    def show(self):
        ...
        self.label_search = tk.Label(self.frame_search, text='Search ID:', font=('Arial', 8), anchor='center', bg='#FFFFFF')
        self.label_search.pack(side='left', padx=5)
        
        self.entry_search = tk.Entry(self.frame_search, font=('Arial', 8), justify='center', width=15, bd=0, textvariable=self.search_result, bg='#FFFFFF', highlightthickness=2)
        self.entry_search.pack(side='left', padx=8)
        
        self.button_go = tk.Button(self.frame_search, command=lambda: self.search_id(self.search_result.get()), text='Go', font=('Arial', 8, 'bold'), width=5, bd=0, bg='#11AFCA', fg='#FFFFFF', activebackground = '#A1A1A1', activeforeground='#FFFFFF')
        ...
        self.button_add = tk.Button(self.frame_top, command=self.add, text='+    Add New', font=('Arial', 8, 'bold'), width=14, bd=0, bg='#81C04B', fg='#FFFFFF', activebackground = '#A1A1A1', activeforeground='#FFFFFF')
    ...
    def show_rows(self, results):
        ...
        for row, key in enumerate(results[self.start:self.start+5]):
            ...
            self.button_view = tk.Button(self.action_buttons, command=lambda key=key: self.view(key), text='View', font=('Arial', 8, 'bold'), width=8, bd=0, bg='#11AFCA', fg='#FFFFFF', activebackground = '#A1A1A1', activeforeground='#FFFFFF')
    ...
    def search_id(self, key):
        self.database = connect('data/students.csv', self.fields, 'id')
        if not key:
            tk.messagebox.showerror('Search Student', 'Field required.')
        elif key not in self.database.keys():
            tk.messagebox.showerror('Search Student', 'Student not found.')
        else:
            self.results = (key,)
            self.entry_count = 1
            self.entry_number = 1
